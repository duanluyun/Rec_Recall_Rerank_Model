...
from Layer.cores import PredictLayer,Similarity
...
def FM(user_feature_columns, item_feature_columns, task='binary'):
    ...
    user_vector_sum = tf.reduce_sum(user_dnn_input, axis=1, keepdims=False)
    item_vector_sum = tf.reduce_sum(item_dnn_input, axis=1, keepdims=False)


    # 计算相似度
    scores = Similarity('cos')([user_vector_sum,item_vector_sum])

    # 确定拟合目标
    output = PredictLayer(task)(scores)
